...
function ArrowLayer({ id, data }: ArrowLayerProps) {
    ...
    img.onload = () => {
      if (!map.hasImage("arrow-head")) {
        try {
          ...
          // Ignore error if another layer added it milliseconds ago
  ...
  // 3. Define Styles with STRICT FILTERS

  // Layer A: The Lines
  const lineStyle: LayerProps = {
    id: lineLayerId,
    type: "line",
    // STRICT FILTER: Only allow LineString or MultiLineString
    filter: [
      "match",
      ["geometry-type"],
    ...
    // We can't easily control z-index across sources, so we rely on component order in LayerManager
  };

  // Layer B: The Arrow Heads
